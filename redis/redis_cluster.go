...
type RedisClientCl struct {
	Config utils.RedisConf
	cc     *gredis.ClusterClient
}

func (c *RedisClientCl) FullKey(key string) string {
	return c.Config.Prefix + key
}

func (c *RedisClientCl) Do(commandName string, args ...interface{}) (interface{}, error) {
	return c.cc.Do(append([]interface{}{commandName}, args...)...).Result()
}
...
func (c *RedisClientCl) SetGlob(key string, ptr interface{}, opt *Option) (error) {
	return SetGlob(c, key, ptr, opt)
}

func (c *RedisClientCl) GetGlob(key string, out interface{}) (error) {
